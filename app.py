...
import requests
import re
from oauth2client.service_account import ServiceAccountCredentials # <--- THIS LINE FIXES YOUR ERROR
        ...
        if st.form_submit_button("🚀 Upload to Cloud"):
            if a > 0:
                try:
                    ...
                    # ID, Date, Type, Category, Amount, Details
                    row = [
                        str(datetime.datetime.now().timestamp()), # Unique ID
                        d.strftime("%Y-%m-%d"), 
                        t, c, a, n
                    ]
                    ws.append_row(row)
